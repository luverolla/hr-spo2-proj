/**
 * Translation of the Arduino MAX32664 library from SparkFun
 */
...
#define CMD_DELAY 20         // Milliseconds
...
#define WRITE_ADDRESS 0xAA
#define READ_ADDRESS 0xAB
...
typedef enum READ_STATUS_BYTE_VALUE {
    SB_SUCCESS = 0x00,
    SB_ILLEGAL_FB_CB = 0x01,
    SB_NOTIMPL_FUNC = 0x02,
    SB_ERR_NUM_FB = 0x03,
    SB_ILLEGAL_CONF = 0x04,
    SB_ERR_MODE = 0x05,
    SB_ERR_GENERAL = 0x80,
    SB_ERR_CHECKSUM = 0x81,
    SB_ERR_AUTH = 0x82,
    SB_ERR_APP = 0x83,
    SB_DEV_BUSY = 0xFE,
    SB_ERR_UNKNOWN = 0xFF
...
// Byte: 0x00.
typedef enum DEVICE_MODE_WRITE_BYTES {
...
// (0x00)
typedef enum OUTPUT_MODE_WRITE_BYTE {
...
typedef struct MAX32664_Handle {
    // Variables ------------
    ...
    GPIO_Line *_resetLine;
    GPIO_Line *_mfioLine;
    uint8_t _address;
    uint32_t _writeCoefArr[3];
    uint8_t _userSelectedMode;
    uint8_t _sampleRate;
    uint8_t readsBuffer[READ_BUF_SIZE];
} MAX32664_Handle;
...
// well as reset.
// INCOMPLETE
uint8_t MAX32664_SetOperatingMode(MAX32664_Handle *handle, uint8_t);
...
uint8_t MAX32664_ConfigBpm(MAX32664_Handle *handle, uint8_t);
...
uint8_t MAX32664_ConfigSensorBpm(MAX32664_Handle *handle, uint8_t);
...
// MAX32664_Width(us) - Resolution -  Sample Rate
...
uint8_t MAX32664_SetPulseWidth(MAX32664_Handle *handle, uint16_t);
...
uint8_t MAX32664_SetSampleRate(MAX32664_Handle *handle, uint16_t);
...
// 32.25pA - 8192nA
// 62.5pA  - 16384nA
uint8_t MAX32664_SetAdcRange(MAX32664_Handle *handle, uint16_t);
...
uint8_t MAX32664_Max30101Control(MAX32664_Handle *handle, uint8_t);
...
uint8_t MAX32664_AccelControl(MAX32664_Handle *handle, uint8_t);

// Family Byte: OUTPUT_MODE (0x10), Index Byte: SET_FORMAT (0x00),
// Write Byte : outputType (Parameter values in OUTPUT_MODE_WRITE_BYTE)
uint8_t MAX32664_SetOutputMode(MAX32664_Handle *handle, uint8_t);
...
// (begin).
uint8_t MAX32664_SetFifoThreshold(MAX32664_Handle *handle, uint8_t);
...
void MAX32664_WriteRegisterMAX30101(MAX32664_Handle *handle, uint8_t, uint8_t);
...
// non-successful write.
void MAX32664_WriteRegisterAccel(MAX32664_Handle *handle, uint8_t, uint8_t);
...
uint8_t MAX32664_ReadRegisterMAX30101(MAX32664_Handle *handle, uint8_t);

// Family Byte: READ_REGISTER (0x41), Index Byte: READ_MAX30101 (0x03), Write Byte:
// Register Address
// This function reads the given register address for the MAX30101 Sensor and
// returns the values at that register.
uint8_t MAX32664_ReadRegisterAccel(MAX32664_Handle *handle, uint8_t);
...
// register value n.
uint8_t MAX32664_DumpRegisterAccelerometer(MAX32664_Handle *handle, uint8_t,
                                           uint8_t regArray[]);
...
// 100 percent.
uint8_t MAX32664_SetAlgoRange(MAX32664_Handle *handle, uint8_t);
...
uint8_t MAX32664_SetAlgoStepSize(MAX32664_Handle *handle, uint8_t);

// Family Byte: CHANGE_ALGORITHM_CONFIG (0x50), Index Byte:
// SET_SENSITIVITY (0x00), Write Byte: AGC_SENSITIVITY_ID (0x02)
// This function changes the sensitivity of the AGC algorithm.
uint8_t MAX32664_SetAlgoSensitivity(MAX32664_Handle *handle, uint8_t);
...
uint8_t MAX32664_SetAlgoSamples(MAX32664_Handle *handle, uint8_t);
...
uint8_t MAX32664_SetMaximFastCoef(MAX32664_Handle *handle, int32_t, int32_t,
                                  int32_t);
...
uint8_t MAX32664_AgcAlgoControl(MAX32664_Handle *handle, uint8_t);

// Family Byte: ENABLE_ALGORITHM (0x52), Index Byte:
// ENABLE_WHRM_ALGO (0x02)
// This function enables (one) or disables (zero) the wrist heart rate monitor
// algorithm.
uint8_t MAX32664_MaximFastAlgoControl(MAX32664_Handle *handle, uint8_t);
...
uint8_t MAX32664_SetNumPages(MAX32664_Handle *handle, uint8_t);
...
uint8_t MAX32664_IsPatientBPMedicationValue(MAX32664_Handle *handle, uint8_t);
...
uint8_t MAX32664_WriteDiastolicVals(MAX32664_Handle *handle, uint8_t, uint8_t,
                                    uint8_t);
...
uint8_t MAX32664_WriteSystolicVals(MAX32664_Handle *handle, uint8_t, uint8_t,
                                   uint8_t);
...
uint8_t MAX32664_IsPatientRestingValue(MAX32664_Handle *handle, uint8_t);
...
uint8_t writeSP02AlgoCoef(MAX32664_Handle *handle, int32_t, int32_t, int32_t);

// Family Byte: CHANGE_ALGORITHM_CONFIG (0x50), Index Byte: BPT_CONFIG (0x04),
// Write Byte: AGC_SP02_COEFS (0x0B)
uint8_t readSP02AlgoCoef(MAX32664_Handle *handle, int32_t userArray[]);

// This function uses the given family, index, and write byte to enable
// the given sensor.
uint8_t MAX32664_EnableWrite(MAX32664_Handle *handle, uint8_t, uint8_t, uint8_t);
...
// transmission.
uint8_t MAX32664_WriteByte(MAX32664_Handle *handle, uint8_t, uint8_t, uint8_t);
...
uint8_t MAX32664_WriteByteParameter(MAX32664_Handle *handle, uint8_t, uint8_t,
                                    uint8_t, uint8_t);
...
// indicating what you want to do, a delay, and then a read to confirm positive
// transmission.
uint8_t MAX32664_WriteByteValue16(MAX32664_Handle *handle, uint8_t, uint8_t,
                                  uint8_t, uint16_t);
...
// of the specific bytes followed by a read to confirm positive transmission.
uint8_t MAX32664_WriteLongBytes(MAX32664_Handle *handle, uint8_t, uint8_t,
                                uint8_t, int32_t _writeVal[], const size_t);

// This function sends information to the MAX32664 to specifically write values
// to the registers of downward sensors and so also requires a
// register address and register value as parameters. Again there is the write
// of the specific bytes followed by a read to confirm positive transmission.
uint8_t MAX32664_WriteBytes(MAX32664_Handle *handle, uint8_t, uint8_t, uint8_t,
                            uint8_t _writeVal[], const size_t);
...
uint8_t MAX32664_ReadByte(MAX32664_Handle *handle, uint8_t, uint8_t, uint8_t *dest);
...
uint8_t MAX32664_ReadByteWrite(MAX32664_Handle *handle, uint8_t, uint8_t,
...
uint16_t MAX32664_ReadIntByte(MAX32664_Handle *handle, uint8_t, uint8_t,
                              uint8_t, uint8_t *dest);
...
uint8_t MAX32664_ReadMultipleBytes32(MAX32664_Handle *handle, uint8_t, uint8_t,
                                     uint8_t, const size_t, int32_t userArray[]);
...
uint8_t MAX32664_ReadMultipleBytes(MAX32664_Handle *handle, uint8_t, uint8_t,
                                   uint8_t, const size_t, uint8_t userArray[]);

// Needs comment - INCOMPLETE
uint8_t MAX32664_ReadFillArray(MAX32664_Handle *handle, uint8_t, uint8_t,
                               uint8_t, uint8_t array[]);
